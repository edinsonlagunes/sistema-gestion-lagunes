...
def asignar_rol(usuario_id: int, data: schemas.AsignarRolRequest, db: Session = Depends(get_db)):
...
def otorgar_permiso_especial(data: schemas.PermisoEspecialCreate, db: Session = Depends(get_db)):
    """
    ...
    """
    if not db.query(models.Usuario).get(data.usuario_id):
        raise HTTPException(status_code=404, detail="Usuario no encontrado")
...
def quitar_permiso_especial(permiso_id: int, db: Session = Depends(get_db)):
